...
import { selectBlock } from 'vue-loader/dist/select';
import { genHotReloadCode } from 'vue-loader/dist/hotReload';
import { genCSSModulesCode } from 'vue-loader/dist/cssModules';
import { formatError } from 'vue-loader/dist/formatError';

import VueLoaderPlugin from 'vue-loader/dist/plugin';
import { canInlineTemplate } from 'vue-loader/dist/resolveScript';
import { setDescriptor } from 'vue-loader/dist/descriptorCache';
...
const exportHelperPath = JSON.stringify(require.resolve('vue-loader/dist/exportHelper'));
...
export default function loader(
    ...
    if (!errorEmitted && !(loaderContext as any)['thread-loader'] && !(loaderContext as any)[VueLoaderPlugin.NS]) {
    ...
    const rawQuery = rawResourceQuery.slice(1);
    const resourceQuery = rawQuery ? '&' + rawQuery : '';
    const incomingQuery = qs.parse(rawQuery);
    const options = (loaderUtils.getOptions(loaderContext) || {}) as VueLoaderOptions;
