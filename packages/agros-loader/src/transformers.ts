...
export const transformEntry: LoaderTransformer = ({
    tree,
    srcPath,
    absolutePath,
}) => {
    // TODO read entry config
    if (!(path.dirname(absolutePath) === srcPath && path.basename(absolutePath) === 'index.ts')) {
        return tree;
    }
...
export const transformComponentDecorator: LoaderTransformer = ({
    tree,
    absolutePath,
}) => {
    if (getCollectionType(absolutePath) !== 'component') {
        return tree;
    }
...
export const transformComponentFile: LoaderTransformer = ({
    tree,
    parsedQuery = {},
}) => {
